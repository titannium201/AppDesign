...
import { getScanReportWithFallback } from '../api';

export function ScanReportScreen() {
  const insets = useSafeAreaInsets();
  const nav = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'ScanReport'>>();
  const [report, setReport] = useState<ScanReportResponse | null>(null);

  useEffect(() => {
    getScanReportWithFallback(route.params.reportId).then(setReport);
